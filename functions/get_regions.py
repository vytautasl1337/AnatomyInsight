...
def get_anatomical_regions(cluster_tables_unc,cluster_tables_corr, atlas_data):
    """
    ...
    """
    atlas_data_ = atlas_data['map'].get_fdata()
    atlas_affine = atlas_data['map'].affine
    region_labels = atlas_data['labels']

    for table in cluster_tables_unc:
    ...
    for table_ in cluster_tables_corr:
        # Add a new column to store the region labels
        table_['Region'] = None
        ...
        for index, row in table_.iterrows():
            ...
            try:
                ...
                # Assign the region label to the current row in the table
                table_.at[index, 'Region'] = region_label
            except:
                table_.at[index, 'Region'] = 'Unknown region'
